class Benutzer:
    ...
    def adaugare_in_fisier(self):
        ok=0
        with open("users.txt", "r") as f:  # citim fisieru
            lines = f.readlines()
            for line in lines:
                line_sep = line.split(' ')
        ...
        if ok==0:
            f = open("users.txt", 'a')
            f.write(str(self.id) + " " + self.nume + " " + self.prenume + " " + "g\n")

    def modificare_prenume(self,key,new_name):
        self.prenume=new_name
        with open("users.txt", 'r') as f:  # modificam si in fisier
            lines = f.readlines()
        with open("users.txt", 'w') as f:
            for line in lines:
                ...
                if line_sep[0] != key:
                    f.write(line)
                if line_sep[0] == key:
                    f.write(str(key) + " " + line_sep[1] + " " + self.prenume + " " + "g\n")

    def delete_benutzer(self, key):
        ...
        with open("users.txt", "r") as f:  # citim fisieru
            lines = f.readlines()
        with open("users.txt", "w") as f:  # rescriem tot fisieru cu tot ce era mai putin linia cu useru pe care vrem sa il stergem
            for line in lines:
                line_sep = line.split(' ')
