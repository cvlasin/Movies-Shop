class film:
    ...
    def add_film(self, titlu, an, nota, actori, pret):
    ...
    def adaugare_in_fisier(self):
        f = open("filme.txt", 'a')
        f.write('\n'+self.titlu + "/" + str(self.an) + "/" + str(self.nota) + "/" + str(self.pret) + "/," + self.actori +",")
        ...
        with open("filme.txt", 'w') as f:
            for line in lines:
                ...
                if line_sep[0] != key:
                    f.write(line)
                if line_sep[0] == key:
                    f.write(line_sep[0]+"/"+line_sep[1]+"/"+line_sep[2]+"/"+str(new_price)+"/"+ line_sep[4])
